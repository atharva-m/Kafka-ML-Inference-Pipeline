...
KAFKA_CONSUMER_GROUP = (
    "benchmark_group"  # Separate from main consumer to avoid message competition
)
        ...
        try:
            ...
            if len(batch_imgs) >= BATCH_SIZE:
                ...
                if metrics.images_processed % 100 == 0:
                    ...
                    logging.info(
                        f"Processed: {metrics.images_processed} | "
...
def benchmark_multi_process(duration_seconds: int) -> BenchmarkMetrics:
    ...
    fetcher = mp.Process(
        target=fetch_process, args=(data_queue, running_event, stats_queue)
    )
...
def print_comparison(single: BenchmarkMetrics, multi: BenchmarkMetrics):
    ...
    print(
        f"{'Images Processed':<25} {single.images_processed:>20} {multi.images_processed:>20}"
    )
    print(
        f"{'Throughput (img/s)':<25} {single.throughput:>20.2f} {multi.throughput:>20.2f}"
    )
    print(f"{'Accuracy (%)':<25} {single.accuracy:>20.2f} {multi.accuracy:>20.2f}")
    print(
        f"{'Avg Latency (ms)':<25} {single.avg_latency_ms:>20.2f} {multi.avg_latency_ms:>20.2f}"
    )
    print(
        f"{'P95 Latency (ms)':<25} {single.p95_latency_ms:>20.2f} {multi.p95_latency_ms:>20.2f}"
    )
    print(
        f"{'P99 Latency (ms)':<25} {single.p99_latency_ms:>20.2f} {multi.p99_latency_ms:>20.2f}"
    )
    print(
        f"{'Avg Inference Time (ms)':<25} {single.avg_inference_time_ms:>20.2f} {multi.avg_inference_time_ms:>20.2f}"
    )
    print(
        f"{'Avg Confidence (%)':<25} {single.avg_confidence:>20.2f} {multi.avg_confidence:>20.2f}"
    )
    print(
        f"{'Min Confidence (%)':<25} {single.min_confidence:>20.2f} {multi.min_confidence:>20.2f}"
    )
    print("-" * 70)
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark Kafka ML Inference Pipeline"
    )
    parser.add_argument(
        "--mode",
